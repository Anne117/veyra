"""Tests for the AgentDojo adapter (Commit 28).

AgentDojoAdapter is a pure data transformation: it converts an already-supplied
AgentDojo-style mapping into a BenchmarkCase. No AgentDojo dependency, no
network/filesystem access, no benchmark/scanner execution, no inference.
"""
...
from veyra.threats.adapters import AgentDojoAdapter
...
def test_valid_full():
    ...
    assert bc.threat_mapping.threat_ids == ("ASI01", "ASI02")


# 3. benchmark_id
def test_benchmark_id():
    assert AgentDojoAdapter.benchmark_id == "agentdojo"
    assert ADAPTER.adapt(_minimal()).benchmark_id == "agentdojo"


# 4. case_id preservation
def test_case_id_preservation():
    assert ADAPTER.adapt(_minimal()).case_id == "ad-001"


# 5. scenario_id == case_id
def test_scenario_id_equals_case_id():
    bc = ADAPTER.adapt(_minimal())
    assert bc.scenario.scenario_id == "ad-001"


# 6. mapping.scenario_id == case_id
def test_mapping_scenario_id_equals_case_id():
    bc = ADAPTER.adapt(_minimal())
    assert bc.threat_mapping.scenario_id == "ad-001"
...
def test_invalid_threat_ids_element_type():
    with pytest.raises(ThreatModelError):
        ADAPTER.adapt({**_minimal(), "threat_ids": ["ASI01", 5]})


def test_invalid_threat_categories_type():
    with pytest.raises(ThreatModelError):
        ADAPTER.adapt({**_minimal(), "threat_categories": "ASI02"})


def test_invalid_attack_behaviors_type():
    with pytest.raises(ThreatModelError):
        ADAPTER.adapt({**_minimal(), "attack_behaviors": 7})


def test_invalid_entry_conditions_element():
    with pytest.raises(ThreatModelError):
        ADAPTER.adapt({**_minimal(), "entry_conditions": ["ok", None]})


def test_invalid_expected_properties_element():
    with pytest.raises(ThreatModelError):
        ADAPTER.adapt({**_minimal(), "expected_security_properties": [{"x": 1}]})
...
def test_serialization_through_serialize_benchmark_case():
    ...
    assert d["benchmark_id"] == "agentdojo"
    assert d["case_id"] == "ad-001"
    assert d["scenario"]["scenario_id"] == "ad-001"
    assert d["scenario"]["threat_categories"] == ["ASI01", "data-exfiltration"]
    assert d["threat_mapping"]["threat_ids"] == ["ASI01", "ASI02"]
...
def test_satisfies_benchmark_adapter_protocol():
    ...
    # Adapt returns a BenchmarkCase, matching the protocol signature.
...
def test_no_execution_network_filesystem(monkeypatch):
    ...
    calls = []
    # If the adapter tried to reach the network it would raise / record a call.
...
def test_no_stdlib_import_side_effects():
    import sys
    bc = ADAPTER.adapt(_full())
    # Pure data: only our model objects in the result.
    assert bc.__class__.__module__ == "veyra.threats.benchmarks"
    assert bc.scenario.__class__.__module__ == "veyra.threats.scenarios"
    assert bc.threat_mapping.__class__.__module__ == "veyra.threats.mapping"
