"""AgentThreatBench adapter (Commit 29).

Converts an **already-supplied** AgentThreatBench-style case representation
into a :class:`veyra.threats.benchmarks.BenchmarkCase`. This is a pure data
transformation only.

It must NOT execute AgentThreatBench, install/download/import AgentThreatBench,
access the network, read the filesystem, run the scanner, build AttackPaths, or
...
This adapter implements only Veyra's explicitly documented small input
contract — not the full AgentThreatBench schema.
"""
...
class AgentThreatBenchAdapter:
    """Converts an AgentThreatBench-style case mapping into a BenchmarkCase.

    Satisfies the :class:`BenchmarkAdapter` protocol.
    """

    benchmark_id = "agentthreatbench"

    def adapt(self, case: Any) -> BenchmarkCase:
        if not isinstance(case, Mapping):
            raise ThreatModelError(
                f"AgentThreatBench case must be a mapping, got {type(case).__name__}"
            )
